...
import { radarData } from "@/lib/radarData";
 ...
 export const metadata = { title: "2D 관제 스코프 | Y&P 레이더" };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold tracking-tight sm:text-3xl">2D 관제 스코프</h1>
          <p className="mt-1.5 max-w-2xl text-sm text-white/55">
            <strong className="text-white/80">{(radarData as { universe?: string }).universe ?? "코스피 200 · 코스닥"}</strong>의 <strong className="text-white/80">온도</strong>(지금 평소와 얼마나 다른가)를
