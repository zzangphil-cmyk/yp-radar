    ...
    <div className="space-y-10">
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between gap-3">
          <h2 className="section-title">
            포지션 맵
            <span className="ml-2 align-middle text-xs font-normal text-white/45">
              매매 × 추정수익
            </span>
          </h2>
          <Link href="/nps/map" className="text-sm link-radar">
            크게 보기 →
          </Link>
        </div>
        <p className="-mt-1 text-sm text-white/55">
          국민연금이 <strong className="text-white/80">사 모은/줄인</strong> 정도(가로)와{" "}
          <strong className="text-white/80">추정 수익률</strong>(세로)로 종목을 배치. 영역을
          클릭하면 종목이 나열됩니다.
        </p>
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between gap-3">
          <h2 className="section-title">
            최근 국민연금 동향{" "}
            <span className="ml-1 align-middle text-xs font-normal text-radar">
              DART 실시간 · {recentDate}
            </span>
          </h2>
          <Link href="/nps/recent" className="text-sm link-radar">
            전체 5%+ 동향 →
          </Link>
        </div>
        ...
        <section className="space-y-3">
          ...
          <div className="card p-5">
            ...
            <p className="mt-4 text-sm text-white/65">
              국내주식 비중 <strong className="text-radar">{dom.pct[0]}%</strong>(
              {allocation.periods[0]}) → <strong className="text-radar">{dom.pct[3]}%</strong>(
              {allocation.periods[3]})로 <strong className="text-white">축소</strong>되는 동안,
              해외주식은 <strong className="text-down">{ovs.pct[0]}%</strong> →{" "}
              <strong className="text-down">{ovs.pct[3]}%</strong>로 확대됐습니다.
            </p>
            <p className="mt-1.5 text-xs text-white/40">
              ※ 2025~2026 국내 비중 반등은 국내증시 급등에 따른 평가액 상승(시장 효과)
              영향이 큽니다. 출처: {allocation.source}. 자료원에 따라 수치 차이가 있을 수
              있습니다.
            </p>
          </div>
        </section>
        ...
        <div className="flex items-end justify-between gap-3 border-t border-white/[0.07] pt-8">
          <div>
            ...
            <p className="mt-1 text-sm text-white/50">
              전체 1,200여 종목 · {changes.curYear}년 말 연간공시 (다음 ~2026년 9월)
            </p>
          </div>
          <Link href="/nps/holdings" className="hidden text-sm link-radar sm:block">
            전체 보기 →
          </Link>
        </div>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
