...
// 테마: 지수·범용 버킷(코스피·대형/코스닥/배당·커버드콜)보다 구체 테마(반도체·2차전지…)를 우선
const GENERIC = new Set(["코스피·대형", "코스닥", "배당·커버드콜", "기타"]);
const themeByCode = {};
etfData.stocks.forEach((s) => { themeByCode[s.code] = (s.themes || []).find((t) => !GENERIC.has(t)) || s.themes?.[0] || "기타"; });
const themeAll = sel.map((p) => themeByCode[p.code] ?? p.theme ?? "기타");
...
const stocks = sel.map((p) => ({ code: p.code, name: p.name, theme: themeByCode[p.code] ?? p.theme, market: marketMap[p.code] }));
const frames = [];
