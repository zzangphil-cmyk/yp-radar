...
const N = P.length;
const pIdxByCode = {}; codes.forEach((c, i) => { pIdxByCode[c] = i; });
...
// 표시 universe: ETF 노출 상위(etf-stocks 순서, =대형주 위주) 중 패널 존재분 상위 TOPN.
//  ※ Object.keys 숫자정렬 함정 회피 — 명시적 노출순으로 선택(삼성전자·SK하이닉스 등 대형주 포함 보장).
const displayIdx = [];
for (const s of etfData.stocks) { const pi = pIdxByCode[s.code]; if (pi != null) { displayIdx.push(pi); if (displayIdx.length >= TOPN) break; } }
const stocks = displayIdx.map((pi) => ({ code: P[pi].code, name: P[pi].name, theme: P[pi].theme }));
const frames = [];
...
for (let fi = 0; fi < FRAMES; fi++) {
  ...
  // 7) 표시 종목(노출 상위 TOPN) blip: [j, x, y, temp, relVol, retPct, d2, topGroup]
  //    j=표시 인덱스(stocks 순서), 값은 P 내 실제 인덱스 pi에서 읽음(D²는 441 단면 기준).
  const b = [];
  for (let j = 0; j < displayIdx.length; j++) {
    const pi = displayIdx[j];
    const x = clamp(Math.log2(Math.max(relVol[pi], 1e-6)) / VOL_EDGE, -1, 1);
    const y = clamp(ret1[pi] / RET_DAILY, -1, 1);
    b[j] = [j, r3(x), r3(y), r2(temp[pi]), r2(relVol[pi]), r2(ret1[pi]), r2(d2[pi]), topG[pi]];
  }
